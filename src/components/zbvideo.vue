          ...
          <el-menu :default-openeds="['1']">
            <el-submenu index="1">
              <template slot="title">英雄联盟</template>
                <el-menu-item v-for="(item,index) in this.$store.state.zbfs" v-if="item.leixing=='英雄联盟'" @click="nihao(item.name)" :key="index">{{item.name}}</el-menu-item>
            </el-submenu>
            <el-submenu index="2">
            ...
            <el-input v-model="url" placeholder="url" clearable></el-input>
            <el-input v-model="pingtai" placeholder="pingtai" clearable></el-input>
            <el-input v-model="leixing" placeholder="leixing" @keyup.enter.native="addzb" clearable></el-input>
          </el-menu>
        </el-aside>
        ...
        addzb(){
            if(this.name==null||this.url==null||this.pingtai==null||this.leixing==null){
              ...
              this.$axios.post('http://web1905131950037.bj01.bdysite.com/public/index.php/index/Index/tjvideozb',"name="+this.name+"&url="+this.url+"&pingtai="+this.pingtai+"&leixing="+this.leixing).then(function(data){
                alert('添加成功。');
                this.name=this.url=this.pingtainull=this.leixing=null;
              });
            }
